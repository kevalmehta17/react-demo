...
export const isFilterStillValid = (
  users: User[],
  appliedFilter: { field: string | null; uniqueVal: string | number },
): boolean => {
  if (
    ...
    appliedFilter.uniqueVal == null
  ) {
    return true;
  }
  return users.some(
    (u) =>
      String(u[appliedFilter.field as keyof User]).toLowerCase() ===
      String(appliedFilter.uniqueVal).toLowerCase(),
  );
};
...
export const shouldResetFilterAfterDelete = (
  ...
  return { resetValue: !isFilterStillValid(remaining, appliedFilter) };
};